...
class Securimage {
     ...
     * @var mixed
     */
    var $gd_font_file = 'font/bubblebath.gdf';
     ...
     * @var string
     */
    var $ttf_file = "font/BIGBUBBA.ttf";
    ...
    function drawWord() {
        if ($this->use_gd_font == true) {
            ...
            for ($i = 0; $i < $strlen; ++$i) {
                ...
                imagettftext($this->im, $this->font_size, $angle, $x, $y, $font_color, Yii::app()->basePath."/components/".$this->ttf_file, $this->code{$i});
...
//function
     ...
     */
    function saveData() {
        Yii::app()->session[$this->sess_name] = strtolower($this->code);
    }
     ...
     */
    function validate() {
        if (isset(Yii::app()->session[$this->sess_name]) && !empty(Yii::app()->session[$this->sess_name])) {
            if (Yii::app()->session[$this->sess_name] == strtolower(trim($this->code_entered))) {
                $this->correct_code = true;
                Yii::app()->session[$this->sess_name] = '';
            } else {
    ...
    function getCode() {
        if (isset(Yii::app()->session[$this->sess_name]) && !empty(Yii::app()->session[$this->sess_name])) {
            return Yii::app()->session[$this->sess_name];
        } else {
            return '';
        }
    }
